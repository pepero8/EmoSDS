...
def extract_samples(csv_data_path, s2u, num_samples=100):
    ...
    # for cur_audio_id, cur_text, cur_style in sampled_data:
    for cur_audio_id, cur_style in sampled_data:
        # convert audio to units
        units = s2u(f"styletalk/audio/{cur_audio_id}")
        sample = {
            # "prefix": f"Predict the style and transcribe following speech input: {units} ",
            "prefix": f"Predict the style of following speech input: {units} ",
            # "plain_text": f"<{cur_style}> {cur_text.strip()}",
            "plain_text": f"<{cur_style}>",
        }
...
if __name__ == "__main__":
    ...
    # Save to JSONL file
    # output_file = "stage3/style_data_eval.jsonl"
    output_file = "stage3/style_data_train.jsonl"
    # output_file = "stage3/asr_style_data_train.jsonl"
    # output_file = "stage2/asr_style_data_train.jsonl"
