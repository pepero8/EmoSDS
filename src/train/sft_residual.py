...
from src.model.llama_custom import LlamaForCausalLMCustom
...
def train():
    ...
    model = LlamaForCausalLMCustom.from_pretrained(
    ...
    residual_id = tokenizer.get_vocab()["<residual>"]
    ...
    def generate_and_tokenize_prompt_residual(data_point):
        ...
        tokenized_full_prompt["residual_id"] = residual_id
        tokenized_full_prompt["residual_embeds"] = residual_embeds
        tokenized_full_prompt["residual_lengths"] = residual_length
    ...
    if training_args.val_set_size > 0:
        ...
        train_val_data = train_val.map(
            generate_and_tokenize_prompt_residual,
            batched=False,
            num_proc=training_args.preprocessing_num_workers,
            load_from_cache_file=True,
            desc=f"generate_and_tokenize_prompt_residual",
        )
        ...
        val_data = valid_data["train"].map(
            ...
            num_proc=training_args.preprocessing_num_workers,
            load_from_cache_file=True,
            desc=f"generate_and_tokenize_prompt_residual_valid",
        )
    else:
        train_data = data["train"].map(
            generate_and_tokenize_prompt_residual,
    ...
    if _test_data is not None:
        test_data = _test_data["train"].map(
            ...
            num_proc=training_args.preprocessing_num_workers,
            load_from_cache_file=True,
            desc=f"generate_and_tokenize_prompt_residual_test",
        )
    else:
        test_data = None
    ...
    def compute_metrics(eval_preds) -> Dict:
        ...
        if training_args.train_task == "asr":
            ...
            # with open(
            #     "/home/jhwan98/EmoSDS/output/for_text_metric_calculation.txt", "a"
            # ) as f:
            #     f.write(f"\n\nres_text_preds: [{'$'.join(response_texts_preds)}]")
            #     f.write(f"\n\nres_text_labels: [{'$'.join(response_texts_labels)}]")
